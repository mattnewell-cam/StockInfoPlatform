...
class Command(BaseCommand):
    help = "Load financials from cached_financials_2.json with new date format."
    ...
    def handle(self, *args, **options):
        ...
        for i, raw_ticker in enumerate(tickers_to_process, 1):
            # Normalize ticker (remove trailing period)
            ...
            for statement in ['IS', 'BS', 'CF']:
                ...
                # Parse dates from headers, skipping first cell (statement name)
                date_info = []
                for h in headers[1:]:
                ...
                for row in statement_data[1:]:
                    ...
                    for val_idx, value_str in enumerate(row[1:]):
                        ...
                        if date_parsed is None:
                            continue  # Skip estimates or unparseable dates

                        month, year, is_estimate, is_ltm = date_parsed

                        if is_ltm:
                            # Handle LTM similar to TTM
                            # Use the FYE month and current/recent year
                            if company.FYE_month:
                                # Find the most recent non-LTM year
                                recent_year = None
                                for di in date_info:
                                    if di and not di[3] and di[1]:  # not LTM and has year
                                        if recent_year is None or di[1] > recent_year:
                                            recent_year = di[1]

                                if recent_year:
                                    # LTM is typically 6 months after the last FY
                            ...
                            # Use the month from the header if available, otherwise FYE month
                            if month and year:
